  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="max-w-3xl mx-auto px-6 mt-2">
        <AdSlot slot="top-banner" label="Advertisement" height={80} />
      </div>
      ...
      <section className="relative px-6 pt-16 pb-20 text-center overflow-hidden">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35, duration: 0.6 }}
          ...
          Three powerful AI tools — one free platform. Generate viral captions, stunning resumes,
          and discover your personality. No signup ever.
        </motion.p>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.5 }}
          ...
          <Link href="#tools" className="btn-outline-glow px-8 py-4 rounded-2xl text-base font-medium">
            View All Tools
          </Link>
        </motion.div>
      </section>
      ...
      <section id="tools" className="max-w-6xl mx-auto px-6 mb-24">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
          ...
          <p className="text-slate-400 max-w-xl mx-auto">No login. No limits. Powerful AI tools ready in seconds.</p>
        </motion.div>
          ...
          {tools.map((tool) => (
            <motion.div key={tool.href} variants={item}>
              <Link href={tool.href} className="group block h-full">
                <div className="glass glass-hover h-full p-7 flex flex-col rounded-2xl relative overflow-hidden">
                  <div className="absolute top-0 left-0 right-0 h-px"
                    style={{ background: `linear-gradient(90deg, transparent, ${tool.accent}60, transparent)` }} />
                  <div className="flex items-center justify-between mb-5">
                    <div className="text-2xl w-12 h-12 glass rounded-xl flex items-center justify-center" style={{ color: tool.accent }}>
                      {tool.icon}
                    </div>
                  ...
                  <h3 className="text-xl font-bold text-white mb-2" style={{ fontFamily: "var(--font-display)" }}>{tool.title}</h3>
                  <p className="text-slate-400 text-sm leading-relaxed flex-1 mb-5">{tool.desc}</p>
      ...
      <div className="max-w-3xl mx-auto px-6 mb-20">
        <AdSlot slot="mid-banner" label="Advertisement" height={250} />
      </div>
      ...
      <section className="max-w-6xl mx-auto px-6 mb-24">
        ...
        <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true }}
          ...
          {[
            ...
            <motion.div key={s.step} variants={item} className="glass rounded-2xl p-6 group hover:border-violet-500/30 transition-all duration-300">
